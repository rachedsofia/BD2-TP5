...
public class JPAVentasService implements VentaService {
   ...
   @SuppressWarnings("rawtypes")
    ...
    public List<Venta> obtenerUltimasVentasCliente(Long idCliente, int limit) {
        return entityManager.createQuery("SELECT v FROM Venta v WHERE v.cliente.id = :idCliente ORDER BY v.fecha DESC", Venta.class)
                .setParameter("idCliente", idCliente)
                .setMaxResults(limit)
                .getResultList();
    }
    ...
    public void registrarVenta(Venta nuevaVenta) {
        entityManager.persist(nuevaVenta);  // Persistimos la venta en la base de datos
    }
}
